...
2. 用户在公司批准的 LorealGPT 中手工粘贴一次完整提示词，由 LorealGPT 同时完成内容规划、页面蓝图和自由 HTML 创作，并把完整源码放在一个可复制的 `html` Markdown 代码块中。
...
- 页面蓝图字段与自由 HTML 约定：LorealGPT 在一次生成中返回页面目标、结论、结构、模块、数据、文案和完整 HTML；最终回复只包含一个 `html` Markdown 代码块，代码块外不输出解释，以便直接使用 Copy code。
...
导入器兼容常见的“HTML 自带翻页播放器”输出：它会按 deck 结构识别真正的 section 页面，忽略导航圆点和翻页控件，并在移除原脚本后让每页继续能够独立编辑和导出。独立 HTML 导出会把项目 CSS 与播放器 CSS 放在带标记的两个 style 块中；播放器只负责缩放和导航，不覆盖项目的文字颜色、字体或页面背景继承，因此深色、浅色和自定义主题保持原样。再次打开时只导入项目 CSS。对旧版本中两类 CSS 混在同一块的文件，导入器也会只清理已知的播放器选择器、深色 body 与居中规则，不会删除普通项目样式。若 LorealGPT 只提供 `data-slide-id`、但页面专属 CSS 使用 `#slide-01`，导入器会自动补齐安全且唯一的 DOM `id`，恢复页面字号、定位和卡片布局；直接位于页面根下的 `.section-label` 会被放到标题上方的安全区，避免两者共享相同的 `top` 坐标而重叠。导入完成后状态栏会明确显示识别页数。
...
v0.2 预览版已完成本机开发与在线验收；三页首页已统一为暖白、粉红和深蓝的产品品牌语言，第三页以独立示例呈现咨询蓝、黑金、暖米三套 PPT 视觉系统。公开仓库与 GitHub Pages 已发布并返回 HTTP 200，仓库只包含工具框架、品牌资源、本地依赖、文档、测试和许可证文件。公司电脑真实环境与网络零内容外传验收尚未完成，因此当前仍标记为测试版本。
